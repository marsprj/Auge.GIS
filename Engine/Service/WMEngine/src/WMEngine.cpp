...
namespace auge
{
	...
	{
		static WebMapEngine g_webMapEngine;
		return &g_webMapEngine;
	}

	WebMapEngine::WebMapEngine():
	m_handler(NULL),
	m_requests("GetCapabilities;GetMap;GetFeatureInfo")
	{
	...
	const char*	WebMapEngine::GetID()
	{
		return "WMEngine";
	}

	const char*	WebMapEngine::GetType()
	{
		return "wms";
	}

	const char*	WebMapEngine::GetDescription()
	{
		return "OGC WMS";
	}

	void* WebMapEngine::GetHandler()
	{
		return m_handler;
	}

	void WebMapEngine::SetHandler(void* handler)
	{
		m_handler = handler;
	}

	const char*	WebMapEngine::GetLibraryPath()
	{
		return NULL;
	}

	WebRequest*	WebMapEngine::ParseRequest(const char* url)
	{
		GPropertySet props;
		props.Parse(url,"&");
	...
	WebResponse* WebMapEngine::Execute(WebRequest* pWebRequest, WebContext* pWebContext, Map* pMap)
	{
		...
		if(g_stricmp(request, "GetCapabilities")==0)
		{
			...
			pWebResponse = GetCapabilities(pGetCapaRequest, pWebContext, pMap);
		}
		else if(g_stricmp(request, "GetMap")==0)
		{
	...
	bool WebMapEngine::Support(const char* request)
	{
		if(request==NULL)
		{
			return false;
