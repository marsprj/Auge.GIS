...
namespace auge
{
	...
	void GServer::Run()
	{
		...
		while(AUGE_FCGI_ACCEPT>=0)
		{
			...
			else
			{
				...
				g_sprintf(msg, "\r[ServiceName]:%s", szServiceName);
				m_pLogger->Info(msg, __FILE__, __LINE__);
			}
	...
	WebResponse* GServer::DoGet(Service* pService, rude::CGI& cgi)
	{
		...
		if(pWebEngine==NULL)
		{
			...
			g_sprintf(msg, "Unsupported Engine [%s]", szService);
			auge::WebExceptionResponse *pExpResponse = NULL;
			pExpResponse = auge::augeCreateWebExceptionResponse();
		...
		pWebRequest = pWebEngine->ParseRequest(cgi, pService->GetMapName());
		if(pWebRequest==NULL)
		{
			// wrong service engine
			auge::WebExceptionResponse *pExpResponse = NULL;
	...
	WebResponse* GServer::DoPost(Service* pService, rude::CGI& cgi)
	{
		...
		//const char* xml_string = "<wfs:GetFeature service=\"WFS\" version=\"1.0.0\"  xmlns:wfs=\"http://www.opengis.net/wfs\" xmlns:ogc=\"http://www.opengis.net/ogc\"  xmlns:world=\"undefined\" xmlns:gml=\"http://www.opengis.net/gml\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://www.opengis.net/wfs http://schemas.opengis.net/wfs/1.0.0/WFS-basic.xsd\"><wfs:Query typeName=\"cities\"><ogc:Filter><ogc:BBOX><gml:Box><gml:coordinates>-180,-146.0625 180,146.0625</gml:coordinates></gml:Box></ogc:BBOX></ogc:Filter></wfs:Query></wfs:GetFeature>";
		...
		pWebRequest = pWebEngine->ParseRequest(pxDoc,pService->GetMapName());
		if(pWebRequest==NULL)
		{
			pxDoc->Close();
			pxDoc->Release();
