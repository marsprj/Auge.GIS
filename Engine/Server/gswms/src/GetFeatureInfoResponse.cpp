...
namespace auge
{
	GetFeatureInfoResponse::GetFeatureInfoResponse(GetFeatureInfoRequest* pRequest)
	{
		m_pRequest = pRequest;
		m_pRequest->AddRef();
	}
	...
	RESULTCODE GetFeatureInfoResponse::Write(WebWriter* pWriter)
	{
		if(pWriter==NULL)
		{
			return AG_FAILURE;
	...
	void GetFeatureInfoResponse::SetPath(const char* path)
	{
		m_path = path;
	}
