...
namespace auge
{
	...
	WebResponse* WMapEngine::Execute(WebRequest* pWebRequest, User* pUser)
	{
	...
	WebResponse* WMapEngine::Execute(WebRequest* pWebRequest, WebContext* pWebContext, User* pUser)
	{
		const char* request = pWebRequest->GetRequest();
		WebHandler* handler = GetHandler(request);
		if(handler == NULL)
