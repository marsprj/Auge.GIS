...
namespace auge
{
	...
	GStatistics* JobManagerImpl::Statistics(const char* field)
	{
		...
		char sql[AUGE_SQL_MAX];
		memset(sql, 0, AUGE_SQL_MAX);
		g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job group by %s", field, field);
	...
	GStatistics* JobManagerImpl::Statistics(const char* field, const char* start_time, const char* end_time)
	{
		...
		char sql[AUGE_SQL_MAX];
		memset(sql, 0, AUGE_SQL_MAX);
		g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where start_time>'%s' and start_time<'%s' group by %s", field, start_time, end_time, field);
		GResultSet* pResult = m_pConnection->ExecuteQuery(sql);
		if(pResult!=NULL)
		{
			g_uint count = 0;
	...
	GStatistics* JobManagerImpl::Statistics(g_int user_id, const char* field)
	{
		...
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where user_id=%d group by %s", field, user_id, field);
		}
	...
	GStatistics* JobManagerImpl::Statistics(g_int user_id, const char* field, const char* start_time, const char* end_time)
	{
		...
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where user_id=%d and start_time>'%s' and start_time<'%s' group by %s", field, user_id, start_time, end_time, field);
		}
	...
	GStatistics* JobManagerImpl::StatisticsByClient(g_int user_id, const char* client, const char* field)
	{
		...
		if(user_id<1)
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where client='%s' group by %s", field, client, field);
		}
		else
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where user_id=%d and client='%s' group by %s", field, user_id, client, field);
		}
	...
	GStatistics* JobManagerImpl::StatisticsByClient(g_int user_id, const char* client, const char* field, const char* start_time, const char* end_time)
	{
		...
		if(user_id<=0)
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where (client='%s' and start_time>'%s' and start_time<'%s') group by %s", field, client, start_time, end_time, field);
		}
		else
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where (client='%s' and user_id=%d and start_time>'%s' and start_time<'%s') group by %s", field, client, user_id, start_time, end_time, field);
		}
	...
	GStatistics* JobManagerImpl::StatisticsByServer(g_int user_id, const char* server, const char* field)
	{
		...
		if(user_id<1)
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where server='%s' group by %s ", field, server, field);
		}
		else
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where user_id=%d and server='%s' group by %s", field, user_id, server, field);
		}
	...
	GStatistics* JobManagerImpl::StatisticsByServer(g_int user_id, const char* server, const char* field, const char* start_time, const char* end_time)
	{
		...
		if(user_id<=0)
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where (server='%s' and start_time>'%s' and start_time<'%s') group by %s", field, server, start_time, end_time, field);
		}
		else
		{
			g_snprintf(sql, AUGE_SQL_MAX, "select %s, count(*) as count from g_job where (server='%s' and user_id=%d and start_time>'%s' and start_time<'%s') group by %s", field, server, user_id, start_time, end_time, field);
		}
