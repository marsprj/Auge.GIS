...
namespace auge
{
	...
	WEngine::WEngine():
	m_handler(NULL),
		m_requests("GetCapabilities;CreateService;RemoveService;UpdateService;GetService;RegisterDataSource;UnRegisterDataSource;")
	{
	...
	WebRequest*	WEngine::ParseRequest(XDocument* pxDoc)
	{
	...
	bool WEngine::Support(const char* request)
	{
		if(request==NULL)
		{
			return false;
