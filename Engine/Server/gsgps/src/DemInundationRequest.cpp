...
namespace auge
{
	std::string DemInundationRequest::DEFAULT_HOST   = "127.0.0.1";
	std::string DemInundationRequest::DEFAULT_METHOD = "gps";

	DemInundationRequest::DemInundationRequest():
	m_version("1.0.0"),
	...
	const char*	DemInundationRequest::GetEngine()
	{
		return "gps";
	}

	const char*	DemInundationRequest::GetVersion()
	{
		return m_version.c_str();
	}

	const char*	DemInundationRequest::GetRequest()
	{
		return "DemInundation";
	}

	void DemInundationRequest::SetVersion(const char* value)
	{
		if(value==NULL)
		{
			return;
	...
	const char*	DemInundationRequest::GetMimeType()
	{
		return m_mime_type.c_str();
	}

	bool DemInundationRequest::Create(rude::CGI& cgi)
	{
		const char* val = NULL;
...
************************************************************************/
	bool DemInundationRequest::Create(XDocument* pxDoc)
	{
	...
	const char*	DemInundationRequest::GetEncoding()
	{
		return m_encoding.c_str();
	}

	void DemInundationRequest::SetUser(const char* user)
	{
		if(user==NULL)
		{
			m_user.clear();
	...
	const char* DemInundationRequest::GetUser()
	{
		return m_user.c_str();
	}

	void DemInundationRequest::SetRequestMethod(const char* method)
	{
		if(method==NULL)
		{
			m_request_method = DEFAULT_METHOD;
	...
	void DemInundationRequest::SetHost(const char* host)
	{
		if(host==NULL)
		{
			m_host = DEFAULT_HOST;
	...
	const char* DemInundationRequest::GetRequestMethod()
	{
		return m_request_method.c_str();
	}

	const char* DemInundationRequest::GetHost()
	{
		return m_host.c_str();
	}

	void DemInundationRequest::SetInputDataSource(const char* sourceName)
	{
		if(sourceName==NULL)
		{
	...
	void DemInundationRequest::SetInputRaster(const char* rasterName)
	{
		if(rasterName==NULL)
		{
	...
	void DemInundationRequest::SetInputPath(const char* rasterPath)
	{
		if(rasterPath==NULL)
		{
			m_in_raster_path = "/";
	...
	void DemInundationRequest::SetOutputDataSource(const char* sourceName)
	{
		if(sourceName==NULL)
		{
	...
	void DemInundationRequest::SetOutputRaster(const char* rasterName)
	{
		if(rasterName==NULL)
		{
	...
	void DemInundationRequest::SetOutputPath(const char* rasterPath)
	{
		if(rasterPath==NULL)
		{
			m_out_raster_path = "/";
	...
	const char*	DemInundationRequest::GetInputDataSource()
	{
		return m_in_source_name.empty() ? NULL : m_in_source_name.c_str();
	}

	const char*	DemInundationRequest::GetInputRaster()
	{
		return m_in_raster_name.empty() ? NULL : m_in_raster_name.c_str();
	}

	const char* DemInundationRequest::GetInputPath()
	{
		return m_in_raster_path.empty() ? NULL : m_in_raster_path.c_str();
	}

	const char*	DemInundationRequest::GetOutputDataSource()
	{
		return m_out_source_name.empty() ? NULL : m_out_source_name.c_str();
	}

	const char*	DemInundationRequest::GetOutputRaster()
	{
		return m_out_raster_name.empty() ? NULL : m_out_raster_name.c_str();
	}

	const char* DemInundationRequest::GetOutputPath()
	{
		return m_out_raster_path.empty() ? NULL : m_out_raster_path.c_str();
	}


	void DemInundationRequest::SetOutputFormat(const char* format)
	{
		if(format==NULL)
		{
	...
	//const char* DemInundationRequest::GetOutputFormat()
	//{
	//	return m_output_format.c_str();
	//}
	
	void DemInundationRequest::Info()
	{
