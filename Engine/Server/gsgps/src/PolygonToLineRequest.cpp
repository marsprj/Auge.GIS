...
namespace auge
{
	std::string PolygonToLineRequest::DEFAULT_HOST   = "127.0.0.1";
	std::string PolygonToLineRequest::DEFAULT_METHOD = "gps";

	PolygonToLineRequest::PolygonToLineRequest():
	m_version("1.0.0"),
	...
	const char*	PolygonToLineRequest::GetEngine()
	{
		return "gps";
	}

	const char*	PolygonToLineRequest::GetVersion()
	{
		return m_version.c_str();
	}

	const char*	PolygonToLineRequest::GetRequest()
	{
		return "PolygonToLine";
	}

	void PolygonToLineRequest::SetVersion(const char* value)
	{
		if(value==NULL)
		{
			return;
	...
	const char*	PolygonToLineRequest::GetMimeType()
	{
		return m_mime_type.c_str();
	}

	bool PolygonToLineRequest::Create(rude::CGI& cgi)
	{
		const char* val = NULL;
...
************************************************************************/
	bool PolygonToLineRequest::Create(XDocument* pxDoc)
	{
	...
	const char*	PolygonToLineRequest::GetEncoding()
	{
		return m_encoding.c_str();
	}

	void PolygonToLineRequest::SetUser(const char* user)
	{
		if(user==NULL)
		{
			m_user.clear();
	...
	const char* PolygonToLineRequest::GetUser()
	{
		return m_user.c_str();
	}

	void PolygonToLineRequest::SetRequestMethod(const char* method)
	{
		if(method==NULL)
		{
			m_request_method = DEFAULT_METHOD;
	...
	void PolygonToLineRequest::SetHost(const char* host)
	{
		if(host==NULL)
		{
			m_host = DEFAULT_HOST;
	...
	const char* PolygonToLineRequest::GetRequestMethod()
	{
		return m_request_method.c_str();
	}

	const char* PolygonToLineRequest::GetHost()
	{
		return m_host.c_str();
	}

	const char* PolygonToLineRequest::GetInputTypeName()
	{
		return m_in_type_name.c_str();
	}

	void PolygonToLineRequest::SetInputTypeName(const char* typeName)
	{
		if(typeName==NULL)
		{
	...
	const char* PolygonToLineRequest::GetInputSourceName()
	{
		return m_in_source_name.empty() ? NULL : m_in_source_name.c_str();
	}

	void PolygonToLineRequest::SetInputSourceName(const char* sourceName)
	{
		if(sourceName==NULL)
		{
	...
	const char* PolygonToLineRequest::GetOutputTypeName()
	{
		return m_out_type_name.c_str();
	}

	void PolygonToLineRequest::SetOutputTypeName(const char* typeName)
	{
		if(typeName==NULL)
		{
	...
	const char* PolygonToLineRequest::GetOutputSourceName()
	{
		return m_out_source_name.empty() ? NULL : m_in_source_name.c_str();
	}

	void PolygonToLineRequest::SetOutputSourceName(const char* sourceName)
	{
		if(sourceName==NULL)
		{
	...
	void PolygonToLineRequest::SetOutputFormat(const char* format)
	{
		if(format==NULL)
		{
	...
	const char* PolygonToLineRequest::GetOutputFormat()
	{
		return m_output_format.c_str();
	}
	
	void PolygonToLineRequest::Info()
	{
