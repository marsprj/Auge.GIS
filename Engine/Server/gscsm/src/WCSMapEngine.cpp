...
namespace auge
{
	...
	{
		static WCSMapEngine g_webCSMapEngine;
		return &g_webCSMapEngine;
	}

	WCSMapEngine::WCSMapEngine()
	{
		m_handler = NULL;
		m_handlers.push_back(new DescribeTaskHandler());
		m_handlers.push_back(new CreateTaskHandler());
		m_handlers.push_back(new JoinTaskHandler());
	}
	...
	void WCSMapEngine::CleanupHandlers()
	{
	...
	WebHandler*	WCSMapEngine::GetHandler(const char* name)
	{
		std::vector<WebHandler*>::iterator iter;
		for(iter=m_handlers.begin(); iter!=m_handlers.end(); iter++)
		{
	...
	const char*	WCSMapEngine::GetID()
	{
		return "WMEngine";
	}

	const char*	WCSMapEngine::GetType()
	{
		return "csm";
	}

	const char*	WCSMapEngine::GetDescription()
	{
		return "CrowdSourcing Map";
	}

	void* WCSMapEngine::GetHandler()
	{
		return m_handler;
	}

	void WCSMapEngine::SetHandler(void* handler)
	{
		m_handler = handler;
	}

	const char*	WCSMapEngine::GetLibraryPath()
	{
		return NULL;
	}

	RESULTCODE WCSMapEngine::Initialize(void *pParam)
	{
		GConnection* pConnection = static_cast<GConnection*>(pParam);
		TaskManager* pTaskManager = augeGetTaskManagerInstance();
		pTaskManager->Initialize(pConnection);
		return AG_SUCCESS;
	}

	WebRequest*	WCSMapEngine::ParseRequest(const char* url)
	{
		//GPropertySet props;
		//props.Parse(url,"&");
	...
	WebRequest* WCSMapEngine::ParseRequest(rude::CGI& cgi)
	{
	...
	WebRequest* WCSMapEngine::ParseRequest(rude::CGI& cgi, const char* mapName)
	{
		const char* request = cgi["request"];
		if(request==NULL)
		{
	...
	WebRequest*	WCSMapEngine::ParseRequest(XDocument* pxDoc, const char* mapName)
	{
		GError* pError = augeGetErrorInstance();
	...
	WebResponse* WCSMapEngine::Execute(WebRequest* pWebRequest, User* pUser)
	{
	...
	WebResponse* WCSMapEngine::Execute(WebRequest* pWebRequest, WebContext* pWebContext, User* pUser)
	{
		const char* request = pWebRequest->GetRequest();
		WebHandler* handler = GetHandler(request);
		if(handler == NULL)
	...
	void WCSMapEngine::Begin(WebRequest* pWebRequest, WebContext* pWebContext, User* pUser)
	{ 
	...
	//WebResponse* WCSMapEngine::Execute(WebRequest* pWebRequest, WebContext* pWebContext, Map* pMap)
	//{
	//	WebResponse	*pWebResponse = NULL;
