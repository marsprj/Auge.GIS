...
namespace auge
{
	...
	RESULTCODE MongoTileStoreGenerator::Execute()
	{
		...
		g_uint r_min=0, r_max=0, c_min=0, c_max=0;
		...
		{
			m_pTileStore->GetBoundingBox(m_viewer, l, r_min, r_max, c_min, c_max);

			//g_uint rows = m_pTileStore->GetRows(l);
			//g_uint cols = m_pTileStore->GetCols(l);

			for(g_uint r=r_min; r<r_max; r++)
			{
				for(g_uint c=c_min; c<c_max; c++)
				{
					...
					if(rc==AG_SUCCESS)
					{
						...
						//sprintf(str,"%d-%d-%d",l,r,c);
						pLogger->Info(str, __FILE__, __LINE__);

						//auge_make_path(t_path, NULL, temp, str,"png");
