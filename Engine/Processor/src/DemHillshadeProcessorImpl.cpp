...
namespace auge
{
	...
	//	for(g_uint i=1; i<height-1; i++)
	//	{	
	//		for(g_int j=1; j<width-1; j++,ptr_0++,ptr_1++,ptr_2++,ptr++)
	//		{
	//			g_int offset = i*width+j;
	...
	RESULTCODE DemHillshadeProcessorImpl::Hillshade_Short(RasterBand* pinBand, RasterBand* poutBand)
	{
		...
		{	
			...
			for(g_int j=1; j<width-1; j++)
			{
				g_int64 offset = 0;

				slope_x = ((ptr_2[offset+1] + 2*ptr_1[offset+1] + ptr_0[offset+1]) - (ptr_2[offset-1] + 2*ptr_1[offset-1] + ptr_0[offset-1])) / (8 * reslution_x);
				slope_y = ((ptr_2[offset+1] + 2*ptr_2[offset+0] + ptr_2[offset-1]) - (ptr_0[offset+1] + 2*ptr_0[offset+0] + ptr_0[offset-1])) / (8 * reslution_y);
				//printf("%f\t%f\n", slope_x, slope_y);
