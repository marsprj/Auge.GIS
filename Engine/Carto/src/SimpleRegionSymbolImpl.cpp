...
namespace auge
{
	SimpleRegionSymbolImpl::SimpleRegionSymbolImpl():
	m_cairo(NULL),
	m_icon(NULL),	
	m_opacity(0.0f),
	m_size(5.0f),
	...
	//////////////////////////////////////////////////////////////////////////
	const char*	SimpleRegionSymbolImpl::GetName()
	{
		return "SimpleRegion";
	}

	const char*	SimpleRegionSymbolImpl::GetIcon()
	{
	...
	void SimpleRegionSymbolImpl::SetStroke(Stroke* pStroke)
	{
		if(m_pStroke!=NULL)
		{
			m_pStroke->Release();
	...
	Stroke* SimpleRegionSymbolImpl::GetStroke()
	{
		return m_pStroke;
	}

	void SimpleRegionSymbolImpl::SetFill(Fill* pFill)
	{
		if(m_pFill!=NULL)
		{
			m_pFill->Release();
	...
	Fill*	SimpleRegionSymbolImpl::GetFill()
	{
		return m_pFill;
	}

	void SimpleRegionSymbolImpl::SetOpacity(float opacity)
	{
		m_opacity = opacity;
	}

	float SimpleRegionSymbolImpl::GetOpacity()
	{
		return m_opacity;
	}

	void SimpleRegionSymbolImpl::SetSize(float size)
	{
		m_size = size;
	}

	float SimpleRegionSymbolImpl::GetSize()
	{
		return m_size;
	}

	void SimpleRegionSymbolImpl::SetRotation(float rotation)
	{
		m_rotation = rotation;
	}

	float SimpleRegionSymbolImpl::GetRotation()
	{
		return m_rotation;
	}

	void SimpleRegionSymbolImpl::Release()
	{
		if(!ReleaseRef())
		{
			delete this;
	...
	augeSymbolizerType SimpleRegionSymbolImpl::GetType()
	{
		return augeSymbolPoint;
	}

	RESULTCODE SimpleRegionSymbolImpl::Draw(Geometry* pGeometry, Renderer* pRenderer, Transformation* pTransform)
	{
		if(pGeometry==NULL)
		{
			return AG_FAILURE;
	...
	void SimpleRegionSymbolImpl::DrawIcon()
	{
	...
	void SimpleRegionSymbolImpl::SaveIcon(const char* icon_path)
	{
		cairo_surface_write_to_png(m_icon, icon_path);
	}

	Symbol*	SimpleRegionSymbolImpl::Clone()
	{
		return (new SimpleRegionSymbolImpl());
	}
}